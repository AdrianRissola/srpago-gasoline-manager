...
public class ExceptionValidationHandler {
	...
	@ExceptionHandler(FuelManagerException.class)
	@ResponseBody
	public final ResponseEntity<ResponseDto> handleFuelManagerException(FuelManagerException fuelManagerException) {
		logger.error("Cause: {}", fuelManagerException.getMessage());
		return new ResponseEntity<ResponseDto>(fuelManagerException.getResponseDto(), fuelManagerException.getHttpStatus());
	}
